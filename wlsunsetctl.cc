...
int main(int argc, char** argv) {
  if (argc == 2 && argv && argv[1] &&
    ...
    absl::PrintF("%d\n", wlsunsetProxy(*connection,
                                       sdbus::ServiceName("org.jjaro.wlsunset"),
                                       sdbus::ObjectPath("/org/jjaro/wlsunset"))
                                 .get_inhibit()
                             ? 1
                             : 0);
    return EXIT_SUCCESS;
    ...
    if (argv[2] && absl::SimpleAtob(argv[2], &arg)) {
      auto connection = sdbus::createSessionBusConnection();
      wlsunsetProxy(*connection, sdbus::ServiceName("org.jjaro.wlsunset"),
                    sdbus::ObjectPath("/org/jjaro/wlsunset"))
    ...
    wlsunsetProxy client(
        *connection, sdbus::ServiceName("org.jjaro.wlsunset"),
        sdbus::ObjectPath("/org/jjaro/wlsunset"), [](bool inhibit) {
          absl::PrintF(
              "{\"text\": \"%1$d\", \"alt\": \"%2$s\", \"tooltip\": \"%2$s\", "
              "\"class\": \"%2$s\"}\n",
    ...
    absl::PrintF(
        "{\"text\": \"%1$d\", \"alt\": \"%2$s\", \"tooltip\": \"%2$s\", "
        "\"class\": \"%2$s\"}\n",
        client.get_inhibit() ? 1 : 0,
        client.get_inhibit() ? "inhibited" : "activated");
